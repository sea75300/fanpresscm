...
final class commentform extends template {
    ...
    public function assignByObject(\fpcm\model\articles\article $article, \fpcm\model\comments\comment $comment, $captcha) : bool
    {
        ...
        $this->setReplacementTags([
            ...
            '{{nameField}}' => (string) (new \fpcm\view\helper\textInput('newcomment[name]'))->setText('')->setClass('fpcm-pub-textinput')->setValue($comment->getName())->setWrapper(false),
            '{{emailDescription}}' => $this->language->translate('GLOBAL_EMAIL'),
            '{{emailField}}' => (string) (new \fpcm\view\helper\textInput('newcomment[email]'))->setText('')->setClass('fpcm-pub-textinput')->setValue($comment->getEmail())->setWrapper(false)->setType('email'),
            '{{websiteDescription}}' => $this->language->translate('COMMMENT_WEBSITE'),
            '{{websiteField}}' => (string) (new \fpcm\view\helper\textInput('newcomment[website]'))->setText('')->setClass('fpcm-pub-textinput')->setValue($comment->getWebsite())->setWrapper(false)->setType('url'),
