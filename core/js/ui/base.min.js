...
fpcm.vars.ui.components.icon[c]:"")?c.replace("{{icon}}",a).replace("{{class}}",b.class).replace("{{spinner}}",b.spinner).replace("{{stack}}",b.stack).replace("{{size}}",b.size).replace("{{text}}",b.text).replace("{{prefix}}",b.prefix?b.prefix:fpcm.vars.ui.components.icon.defaultPrefix):""},getTextInput:function(a){if(!a||!a.name)return console.warn("Invalid input params given!"),"";a.id||(a.id=a.name);void 0===a.value&&(a.value="");void 0===a.text&&(a.text="");void 0===a.placeholder&&(a.placeholder=
"");void 0===a.maxlenght&&(a.maxlenght="255");void 0===a.type&&(a.type="text");void 0===a.class&&(a.class="");console.log(a);return fpcm.vars.ui.components.input.replace("{{name}}",a.name).replace("{{id}}",a.id).replace("{{id}}",a.id).replace("{{value}}",a.value).replace("&lbrace;&lbrace;value&rcub;&rcub;",a.value).replace("{{type}}",a.type).replace("{{text}}",a.text).replace("{{class}}",a.class).replace("{{type}}",a.type).replace("{{placeholder}}",a.placeholder).replace("{{maxlenght}}",a.maxlenght)},
initDateTimeMasks:function(){if(!fpcm.vars.jsvars.dtMasks)return!1;fpcm.ui.autocomplete("#system_dtmask",{source:fpcm.vars.jsvars.dtMasks});fpcm.ui.autocomplete("#usermetasystem_dtmask",{source:fpcm.vars.jsvars.dtMasks})},initTabsScroll:function(a,b){var c=fpcm.dom.fromTag(a);a=c.find("ul.ui-tabs-nav");var d=c.find("div.fpcm-tabs-scroll").width();c=c.find("li.ui-tabs-tab");var e=0;jQuery.each(c,function(a,b){e+=fpcm.dom.fromTag(b).width()+5});if(a.data("fpcmtabsscrollinit")&&!b)return!0;a.data("fpcmtabsscrollinit",
1);if(e<=d)return a.width("auto"),!1;a.width(parseInt(e));return!0},getCheckboxCheckedValues:function(a){var b=[];fpcm.dom.fromTag(a+":checked").map(function(a,d){b.push(d.value)});return b},getValuesByClass:function(a,b){a=fpcm.dom.fromClass(a);if(!a.length)return{};_data=b?[]:{};a.map(function(a,d){a=fpcm.dom.fromTag(d);if(b)return _data.push(a.val()),!0;_data[a.attr("name")]=a.val();return!0});return _data},confirmDialog:function(a){var b=fpcm.ui.getDialogSizes(top,.35);a.clickNoDefault&&(a.clickNo=
function(){fpcm.dom.fromTag(this).dialog("close");return!1});fpcm.ui.dialog({title:fpcm.ui.translate("GLOBAL_CONFIRM"),content:fpcm.ui.translate("CONFIRM_MESSAGE"),dlWidth:b.width,dlButtons:[{text:fpcm.ui.translate("GLOBAL_YES"),icon:"ui-icon-check",class:a.defaultYes?"fpcm-ui-button-primary":"",click:a.clickYes},{text:fpcm.ui.translate("GLOBAL_NO"),icon:"ui-icon-closethick",class:a.defaultNo?"fpcm-ui-button-primary":"",click:a.clickNo}]})},insertDialog:function(a){var b={id:a.id,title:fpcm.ui.translate(a.title),
dlWidth:a.dlWidth};void 0!==a.dlHeight&&(b.dlHeight=a.dlHeight);void 0!==a.resizable&&(b.resizable=a.resizable);b.dlButtons=a.dlButtons?a.dlButtons:[];a.insertAction&&b.dlButtons.push({text:fpcm.ui.translate("GLOBAL_INSERT"),icon:"ui-icon-check",class:"fpcm-ui-button-primary",click:a.insertAction});a.fileManagerAction&&b.dlButtons.push({text:fpcm.ui.translate("HL_FILES_MNG"),icon:"ui-icon-folder-open",click:a.fileManagerAction});a.closeAction||(a.closeAction=function(){fpcm.dom.fromTag(this).dialog("close")});
b.dlButtons.push({text:fpcm.ui.translate("GLOBAL_CLOSE"),icon:"ui-icon-closethick",click:a.closeAction});a.dlOnOpen&&(b.dlOnOpen=a.dlOnOpen);a.dlOnClose&&(b.dlOnClose=a.dlOnClose);a.onCreate&&(b.onCreate=a.onCreate);return fpcm.ui.dialog(b)},initPager:function(a){void 0===a&&(a={});if(!fpcm.vars.jsvars.pager)return!1;var b=fpcm.dom.fromId("pagerBack"),c=fpcm.dom.fromId("pagerNext");b.unbind("click");c.unbind("click");a.backAction||(a.backAction=function(){fpcm.dom.fromTag(this).attr("href",fpcm.vars.jsvars.pager.linkString.replace("__page__",
fpcm.vars.jsvars.pager.showBackButton))});a.nextAction||(a.nextAction=function(){fpcm.dom.fromTag(this).attr("href",fpcm.vars.jsvars.pager.linkString.replace("__page__",fpcm.vars.jsvars.pager.showNextButton))});fpcm.vars.jsvars.pager.showBackButton?(b.removeClass("fpcm-ui-readonly"),b.click(a.backAction)):fpcm.vars.jsvars.pager.showBackButton||!b||b.hasClass("fpcm-ui-readonly")||b.addClass("fpcm-ui-readonly");fpcm.vars.jsvars.pager.showNextButton?(c.removeClass("fpcm-ui-readonly"),c.click(a.nextAction)):
fpcm.vars.jsvars.pager.showNextButton||!c||c.hasClass("fpcm-ui-readonly")||c.addClass("fpcm-ui-readonly");b=fpcm.dom.fromId("pageSelect");b.unbind("select");a.keepSelect||b.empty();if(fpcm.vars.jsvars.pager.maxPages)for(i=1;i<=fpcm.vars.jsvars.pager.maxPages;i++)b.append("<option "+(fpcm.vars.jsvars.pager.currentPage===i?"selected":"")+' value="'+i+'">'+fpcm.ui.translate("GLOBAL_PAGER").replace("{{current}}",i).replace("{{total}}",fpcm.vars.jsvars.pager.maxPages)+"</option>");a.selectAction||(a.selectAction=
function(a,b){if(b.item.value==fpcm.vars.jsvars.pager.currentPage)return!1;if("1"==b.item.value)return window.location.href=fpcm.vars.actionPath+fpcm.vars.jsvars.currentModule,!0;window.location.href=fpcm.vars.actionPath+fpcm.vars.jsvars.currentModule+"&page="+b.item.value});fpcm.ui.selectmenu("#pageSelect",{width:"auto",select:a.selectAction,classes:{"ui-selectmenu-button":"fpcm-ui-pager-element"},doRefresh:!0})},relocate:function(a){"self"===a&&(a=window.location.href);window.location.href=a},openWindow:function(a){return window.open(a)},
updateMainToolbar:function(a){var b=a.newTab?a.newTab:a.tab;a=a.oldTab?fpcm.dom.fromTag(a.oldTab).data("toolbar-buttons"):1;b=fpcm.dom.fromTag(b).data("toolbar-buttons");fpcm.ui.mainToolbar.find(".fpcm-ui-maintoolbarbuttons-tab"+a).addClass("fpcm-ui-hidden");fpcm.ui.mainToolbar.find(".fpcm-ui-maintoolbarbuttons-tab"+b).removeClass("fpcm-ui-hidden");fpcm.ui.controlgroup(fpcm.ui.mainToolbar,"refresh")},resetSelectMenuSelection:function(a){fpcm.dom.resetValuesByIdsSelect([a]);return!0},showCurrentPasswordConfirmation:function(){var a=
fpcm.dom.fromId("fpcm-ui-currentpass-box");if(!a.length)return!1;a.removeClass("fpcm-ui-hidden");return!0},isHidden:function(a){a instanceof Object||(a=fpcm.dom.fromTag(a));return a.hasClass("fpcm-ui-hidden")?!0:!1},getUniqueID:function(a){return(new Date).getMilliseconds()+Math.random().toString(36).substr(2,9)+(a?a:"")},setFocus:function(a){console.warn("fpcm.ui.setFocus is deprecated and will be removed shortly. Use fpcm.dom.setFocus instead!");fpcm.dom.setFocus(a)},assignHtml:function(a,b){console.warn("fpcm.ui.assignHtml is deprecated and will be removed shortly. Use fpcm.dom.assignHtml instead!");
