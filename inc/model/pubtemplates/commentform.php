...
final class commentform extends template {
    ...
    public function assignByObject(\fpcm\model\articles\article $article, \fpcm\model\comments\comment $comment, $captcha) : bool
    {
        ...
        $this->setReplacementTags([
            ...
            '{{emailField}}' => (string) (new \fpcm\view\helper\textInput('newcomment[email]'))->setClass('fpcm-pub-textinput')->setValue($comment->getEmail())->setWrapper(false)->setType('email'),
            '{{websiteDescription}}' => $this->language->translate('COMMMENT_WEBSITE'),
            '{{websiteField}}' => (string) (new \fpcm\view\helper\textInput('newcomment[website]'))->setClass('fpcm-pub-textinput')->setValue($comment->getWebsite())->setWrapper(false)->setType('url'),
