            ...
            foreach ($articles as $articleId => $article) {
                ...
                $title = [
                    ...
                    $this->lang->translate('EDITOR_AUTHOREDIT', [
                        '{{username}}' => isset($this->users[$article->getCreateuser()])
                                        ? $this->users[$article->getCreateuser()]->getDisplayname()
                    ...
                    $this->lang->translate('EDITOR_LASTEDIT', [
                        '{{username}}' => isset($this->users[$article->getChangeuser()])
                                        ? $this->users[$article->getChangeuser()]->getDisplayname()
                                        : $this->lang->translate('GLOBAL_NOTFOUND'),
                ...
                $dataView->addRow(
                    new \fpcm\components\dataView\row([
                        new \fpcm\components\dataView\rowCol('select', (string) (new \fpcm\view\helper\checkbox('actions[' . $nameList . '][]', 'chbx' . $articleId))->setClass('fpcm-ui-list-checkbox fpcm-ui-list-checkbox-subitem' . $articleMonth)->setValue($articleId)->setReadonly(!$article->getEditPermission()), '', \fpcm\components\dataView\rowCol::COLTYPE_ELEMENT),
                        new \fpcm\components\dataView\rowCol('button', implode('', $buttons), '', \fpcm\components\dataView\rowCol::COLTYPE_ELEMENT),
                        new \fpcm\components\dataView\rowCol('title', implode('<br>', $title) , 'fpcm-ui-ellipsis'),
                        new \fpcm\components\dataView\rowCol('categories', implode(', ', $article->getCategories())),
                        new \fpcm\components\dataView\rowCol('metadata', implode('', $metaData), '', \fpcm\components\dataView\rowCol::COLTYPE_ELEMENT),
                    ]
                ));
            }
        }
