<table class="fpcm-ui-table fpcm-ui-modules">
    ...
    <?php \fpcm\view\helper::notFoundContainer($modules, 6); ?>
    ...
    <tr class="fpcm-ui-modules-updates<?php print $module->hasUpdates(); ?>">
        ...
        <?php if ($module->isInstalled()) : ?>
            <?php if ($module->dependenciesOk() && $module->currentLanguageIncluded()) : ?>
                <?php if ($module->getStatus()) : ?>
                    <?php \fpcm\view\helper::linkButton($theView->basePath.'modules/config&key='.$module->getkey(), 'MODULES_LIST_CONFIGURE', str_replace('/', '', $module->getKey()), 'fpcm-ui-button-blank fpcm-modules-configuremodule fpcm-loader'); ?>
                    <?php if ($permissionEnable) : ?>
                        <?php \fpcm\view\helper::linkButton('#', 'MODULES_LIST_DISABLE', str_replace('/', '', $module->getKey()), 'fpcm-ui-button-blank fpcm-modulelist-singleaction-disable'); ?>
                    <?php endif; ?>
                <?php else : ?>
                    <?php \fpcm\view\helper::dummyButton('MODULES_LIST_CONFIGURE', 'fpcm-ui-button-blank fpcm-modules-configuremodule fpcm-ui-readonly'); ?>
                    <?php if ($permissionEnable) : ?>
                        <?php \fpcm\view\helper::linkButton('#', 'MODULES_LIST_ENABLE', str_replace('/', '', $module->getKey()), 'fpcm-ui-button-blank fpcm-modulelist-singleaction-enable'); ?>
                    <?php endif; ?>           
                <?php endif; ?>
            <?php elseif (!$module->currentLanguageIncluded()) : ?>    
                <?php \fpcm\view\helper::dummyButton('MODULES_FAILED_LANGUAGE', 'fpcm-ui-button-blank fpcm-modules-depencerror'); ?>
            <?php elseif (!$module->dependenciesOk()) : ?>    
                <?php \fpcm\view\helper::dummyButton('MODULES_FAILED_DEPENCIES', 'fpcm-ui-button-blank fpcm-modules-depencerror'); ?>
            <?php endif; ?>

            <?php if ($permissionUninstall) : ?>
                <?php if ($module->getStatus()) : ?>
                    <?php \fpcm\view\helper::dummyButton('MODULES_LIST_UNINSTALL', 'fpcm-ui-button-blank fpcm-modulelist-singleaction-uninstall fpcm-ui-readonly'); ?>
                <?php else : ?>
                    <?php \fpcm\view\helper::linkButton('#', 'MODULES_LIST_UNINSTALL', str_replace('/', '', $module->getKey()), 'fpcm-ui-button-blank fpcm-modulelist-singleaction-uninstall'); ?>
                <?php endif; ?>
            <?php endif; ?>
            <?php if ($permissionInstall && $module->hasUpdates()) : ?>
                <?php \fpcm\view\helper::linkButton('#', 'MODULES_LIST_UPDATE', str_replace('/', '', $module->getKey()), 'fpcm-ui-button-blank fpcm-modulelist-singleaction-update'); ?>
            <?php endif; ?>
        ...
        <td><?php fpcm\model\view\helper::linkButton('', \fpcm\view\helper::escapeVal($module->getName()), str_replace('/', '', $module->getKey()), 'fpcm-module-openinfo-link'); ?></td>
