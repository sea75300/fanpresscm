...
*/
if(void 0===fpcm)var fpcm={vars:{jsvars:{},ui:{messages:[]}},modules:{}};
...
d=a.selectionEnd;b=a.value.substring(c,d);a.value=a.value.substr(0,c)+aTag+b+eTag+a.value.substr(d);c=0==b.length?c+aTag.length:c+aTag.length+b.length+eTag.length;a.selectionStart=c;a.selectionEnd=c}else{for(d=/^[0-9]{0,3}$/;!d.test(c);)c=prompt("Einf\u00fcgen an Position (0.."+a.value.length+"):","0");c>a.value.length&&(c=a.value.length);a.value=a.value.substr(0,c)+aTag+b+eTag+a.value.substr(c)}},addMessage:function(a){jQuery("#fpcm-messages").append('<div class="fpcm-pub-message-box fpcm-pub-message-'+
