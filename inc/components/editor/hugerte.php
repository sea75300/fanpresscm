 ...
 * @license http://www.gnu.org/licenses/gpl.txt GPLv3
 */
class hugerte extends articleEditor {
    ...
    {
        return \fpcm\classes\dirs::getCoreDirPath(\fpcm\classes\dirs::CORE_VIEWS, 'articles/editors/hugerte.php');
    }
    ...
    {
        return \fpcm\classes\dirs::getCoreDirPath(\fpcm\classes\dirs::CORE_VIEWS, 'comments/editors/hugerte.php');
    }
    ...
    {
        return [\fpcm\classes\loader::libGetFileUrl('hugerte/hugerte.min.js'), 'editor/hugerte.js', 'editor/filemanager.js'];
    }
    ...
    public function getJsVars()
    {
        ...
        } elseif ($this->cache->isExpired('hugerte_plugins')) {

            $path = dirname(\fpcm\classes\loader::libGetFilePath('hugerte/hugerte.min.js'));
            $path .= '/plugins/*';

            $pluginFolders = array_map('basename', glob($path, GLOB_ONLYDIR));
            $this->cache->write('hugerte_plugins', $pluginFolders, $this->config->system_cache_timeout);
        } else {
            $pluginFolders = $this->cache->read('hugerte_plugins');
        }

        $cssClasses = array_merge($editorStyles, $this->getEditorStyles());

        $cfg = [
            'editorConfig' => new conf\hugerte(
                $this->config,
                $pluginFolders,
                $cssClasses,
        ...
        $ev = $this->events->trigger('editor\initHugerte', $cfg);
        if (!$ev->getSuccessed() || !$ev->getContinue()) {
            trigger_error(sprintf("Event editor\initHugerte failed. Returned success = %s, continue = %s", $ev->getSuccessed(), $ev->getContinue()));
            return $cfg;
        }
    ...
    /**
     * Arary mit Informationen u. a. für template-Plugin von HugeRTE
